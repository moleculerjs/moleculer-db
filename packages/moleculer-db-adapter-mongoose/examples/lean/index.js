...
broker.createService(StoreService, {
	name: "posts",
	adapter: new MongooseAdapter("mongodb://localhost/moleculer-db-demo", null, {
		useLean: true
	}),
	model: Post,
	settings: {},
...
const isLeanDoc = (obj) => typeof obj === "object" && !obj.save;
...
checker.add("FIND", () => adapter.find({}), res => {
	console.log(res);
	return res.length == 1 && res[0]._id.toHexString() == ids[0] && isLeanDoc(res);
});

// Find by ID
checker.add("GET", () => adapter.findById(ids[0]), res => {
	console.log(res);
	return res._id.toHexString() == ids[0] && isLeanDoc(res);
});
...
checker.add("FIND by query", () => adapter.find({ query: { title: "Last" } }), res => {
	console.log(res);
	return res.length == 1 && res[0]._id.toHexString() == ids[2] && isLeanDoc(res[0]);
});

// Find
checker.add("FIND by limit, sort, query", () => adapter.find({ limit: 1, sort: ["votes", "-title"], offset: 1 }), res => {
	console.log(res);
	return res.length == 1 && res[0]._id.toHexString() == ids[0] && isLeanDoc(res[0]);
});
...
checker.add("FIND by text search", () => adapter.find({ search: "content" }), res => {
	...
	return [
		res.length == 2,
		!isLeanDoc(res[0]),
		res[0]._doc._score < 1 && res[0].title === "Hello",
		res[1]._doc._score < 1 && res[1].title === "Second"
	];
});
...
checker.add("GET BY IDS", () => adapter.findByIds([ids[2], ids[0]]), res => {
	console.log(res);
	return res.length == 2  && isLeanDoc(res[0]);
});
